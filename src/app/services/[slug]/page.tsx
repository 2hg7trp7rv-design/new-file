    ...
    <main>
      <section className="detail-hero">
        <div className="shell breadcrumb">
          <Link href="/">HOME</Link><span>/</span><span>{service.name}</span>
        </div>
      ...
      <section className="price-section section shell" aria-labelledby="price-title">
        ...
        <div className="price-table-wrap" tabIndex={0} aria-label="横にスクロールできる料金表">
          <table className="price-table">
            <thead><tr>{service.pricing.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead>
            <tbody>
              {service.pricing.rows.map((row) => (
                <tr key={row.join("-")}>{row.map((cell, index) => index === 0 ? <th key={cell}>{cell}</th> : <td key={`${cell}-${index}`}>{cell}</td>)}</tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <section className="detail-bottom-cta">
        <div className="shell">
          ...
          <a className="button button-light" href={SITE.reserveUrl} target="_blank" rel="noreferrer">空き状況を見る <ArrowRightIcon /></a>
        </div>
      </section>
    </main>
  );
